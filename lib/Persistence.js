...
class Persistence extends EventEmitter {
    ...
    _reset(){
        ...
        const ormConfig = this.config.orm;
        const ormTimeout = this.config.timeout;
        ...
        this.orm.connect = function(config={}) {
            ...
            return new Promise((resolve, reject) => {
                ...
                this.initialize(config, (err, orm)=> {
                    ...
                    if(err) {
                        _logger.error('ORM initialize error...', err.message);
                        reject(err);
                    } else resolve(orm);
                });
            });
    ...
    reload() {
        if(this.reloading) {
            this.logger.warn('The Persistence module is currently reloading.');
            this.logger.warn('We are trying to reload again... too soon!');
            
            //TODO: We could add a listener and retrigger the reload?
            // return new Promise((resolve, reject)=>{
            //     this.once('persistence.reloaded', ()=>{
            //         resolve(this.reload());
            //     });    
            // });
        ...
        this.logger.info('Start reload routine....');
        ...
        return this.loadDir(this.modelsDir).then((identities) => {
            ...
            return this.orm.connect().then((orm) => {
                ...
                if(this.reloading) {
                    ...
                    this.emit('persistence.reloaded', orm);
                } else this.emit('persistence.ready', orm);
    ...
    loadDir(dir) {
        ...
        return fs.readdirAsync(dir).each((file)=>{
             ...
             * - models.activitylog
             */
            if(!m.prototype.exportName){ 
                m.prototype.exportName = filename;
            }
    ...
    loadModel(id, model){
        ...
        /**
         * TODO: waterline 0.13 has deprecated this 
         * method use `registerModel`
         */
