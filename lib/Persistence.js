    ...
    connect() {
        return this.loadDir(this.modelsDir).then(identities => {
            ...
            return this._connect({ models }).then(orm => {
                ...
                this.logger.info('Datastores: %j', orm.datastores.development)
...
function getExportName(identity, collections) {
    const defintion = collections[identity];
    if (definition.globalId) return defintion.globalId;
    if (defintion.exportName) return definition.exportName;
