...
const _invalidate = require('invalidate-module');
...
class Persistence extends EventEmitter {
    ...
    init(options) {
        ...
        const ormConfig = options.orm;

        this._validateORMConfig(ormConfig);

        const ormTimeout = options.timeout;
        ...
        return new Promise((resolve, reject)=>{
            try {
                this.orm.teardown((err)=> {
                    ...
                    let c = this.orm.connect;
                    this.orm = new Waterline();
                    this.orm.connect = c.bind(this.orm);
